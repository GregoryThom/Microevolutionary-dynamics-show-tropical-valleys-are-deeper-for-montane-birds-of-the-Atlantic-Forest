#Running niche models with chelsa data
library(maptools)
library(ENMeval)
library(rgeos)
library(spocc)
...
pnewmap <- getMap(resolution = "coarse")  
...
occurrences <- read.csv("1_All_sp_filtered.csv",header=T)[1:3]
...
#     ENMevaluate     #
#######################
#Jacknife can be used when less than 20 samples are available
