...
setwd("~/Dropbox (ecoevounifesp)/1_pos_DOC/desenvolvimento/1_paper_22sp/dados_espaciais/")
...
data_part <- data_total[grep("ventralis", data_total$name),]
data_part2 <- SpatialPoints(data_part[,2:3])
plot(climatelayers_ss_cropped[[1]],add=F)
plot(data_part2, add=T)
plot(pnewmap, add=T)


#points <- SpatialPoints(data[,c('longitude', 'latitude')])
#plot(r3[[4]])
#plot(points,add=T)
#vallues <- extract(r3, points)[,1:19]
#rownames(vallues) <- data$pop
climatelayers <- predsProj
climatelayers_n <- raster::scale(climatelayers)

plot(climatelayers)
plot(climatelayers_n)
...
wrwrite.csv(niche_overlap, "../8_PGLS/Niche_hypervolume_all_info.csv")
write.csv(pca_vari, "variaveis coletadas N e S/PCA_Niche_hypervolume.csv")
...
vol_all_sp2$`as.matrix(volumes)[1:2, ]` <- log(vol_all_sp2$`as.matrix(volumes)[1:2, ]`)
write.csv(vol_all_sp2, "variaveis coletadas N e S/log_Niche_hypervolume.txt")
   pdf("1_hypervolume_nicho.pdf")
