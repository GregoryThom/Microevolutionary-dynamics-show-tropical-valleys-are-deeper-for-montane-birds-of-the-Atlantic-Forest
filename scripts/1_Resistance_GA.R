...
setwd("~/Dropbox/1_pos_DOC/desenvolvimento/1_paper_22sp/7_ResistanceGA/")
files <- list.dirs("../2_conStruct/1_no_spatial/", full.names=F, recursive=FALSE)
files <- files[18:22]
#file <- files
for(file in files){ 
  
  dgen <- as.dist(read.csv(paste("../7_Comon_analyses/",file, ".gen_dist", sep = ""), sep = " "))
 ...
 #plot(saz)
  ...
  write.dir <- paste("/home/gthom/Dropbox/1_pos_DOC/desenvolvimento/1_paper_22sp/7_ResistanceGA/", file,"/",file, "/",sep = "")
